...
SELECT
    ...
    COUNT(*) FILTER (
        WHERE DAYS_TO_CONVERT <= 1
    ) AS CONVERTED_WITHIN_1_DAY,
    COUNT(*) FILTER (
        WHERE DAYS_TO_CONVERT <= 3
    ) AS CONVERTED_WITHIN_3_DAYS,
    COUNT(*) FILTER (
        WHERE DAYS_TO_CONVERT <= 7
    ) AS CONVERTED_WITHIN_1_WEEK,
    COUNT(*) FILTER (
        WHERE DAYS_TO_CONVERT <= 30
    ) AS CONVERTED_WITHIN_1_MONTH,
    ROUND(
        COUNT(*) FILTER (
            WHERE DAYS_TO_CONVERT <= 3
        ) / NULLIF(COUNT(*), 0) * 100,
