...
SELECT
    ...
    ROUND((cs.total_revenue - cs.total_spend) / NULLIF(cs.total_spend, 0) * 100, 2) AS roi_percentage,
    ROUND(cs.leads_generated / NULLIF(cs.unique_visitors, 0) * 100, 2) AS visit_to_lead_rate,
    ROUND(cs.purchases_completed / NULLIF(cs.leads_generated, 0) * 100, 2) AS lead_to_purchase_rate,
    ROUND(cs.total_spend / NULLIF(cs.unique_visitors, 0), 2) AS cpu,
    ROUND(cs.total_spend / NULLIF(cs.leads_generated, 0), 2) AS cpl,
    ROUND(cs.total_spend / NULLIF(cs.purchases_completed, 0), 2) AS cppu,
...
WITH lead_conversion AS (
    SELECT
        ...
        (l.created_at::date - MIN(s.visit_date)) AS days_to_convert
    FROM leads l
    JOIN sessions s ON l.visitor_id = s.visitor_id
    GROUP BY l.visitor_id, l.lead_id, l.created_at, l.closing_reason, l.status_id
)
...
SELECT
    ...
    ROUND(COUNT(*) FILTER (WHERE days_to_convert <= 3) * 100.0 / NULLIF(COUNT(*), 0), 2) AS pct_converted_in_3_days,
    ROUND(COUNT(*) FILTER (WHERE closing_reason = 'Успешно реализовано') * 100.0 / NULLIF(COUNT(*), 0), 2) AS success_rate
...
SELECT
    ...
    ROUND((SUM(revenue) - SUM(total_cost)) / NULLIF(SUM(total_cost), 0) * 100, 2) AS weekly_roi,
    ROUND(SUM(leads_count) * 100.0 / NULLIF(COUNT(DISTINCT visitor_id), 0), 2) AS weekly_conversion_rate
FROM marketing_data
GROUP BY week_start, utm_source
ORDER BY week_start, weekly_roi DESC;
...
SELECT
    ...
    CASE
        WHEN (SUM(revenue) - SUM(total_cost)) / NULLIF(SUM(total_cost), 0) * 100 > 50 THEN 'Увеличить бюджет'
        WHEN (SUM(revenue) - SUM(total_cost)) / NULLIF(SUM(total_cost), 0) * 100 BETWEEN 0 AND 50 THEN 'Поддерживать текущий уровень'
        WHEN (SUM(revenue) - SUM(total_cost)) / NULLIF(SUM(total_cost), 0) * 100 < 0 THEN 'Сократить или оптимизировать'
